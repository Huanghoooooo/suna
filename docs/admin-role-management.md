...
|---|---|
| `/admin` | 概览面板 Overview |
| `/admin/accounts` | 账号列表 + 平台角色概览，可搜索 + 分页 |
| `/admin/accounts/[id]` | 账号详情 + 平台角色下拉 + 账号内成员管理 |
...
│   │   ├── page.tsx                         # 概览
│   │   ├── accounts/
│   │   │   ├── page.tsx                     # 账号列表（搜索 + 分页 + 平台角色徽标）
│   │   │   └── [id]/page.tsx                # 账号详情 + 平台角色下拉 + 成员表
│   │   ├── sandboxes/
│   │   ├── analytics/
│   │   ├── access-requests/
│   │   ├── feedback/
...
│   │   ├── stress-test/
│   │   └── sandbox-pool/
│   ├── components/admin/                    # 账号/角色相关组件直接内联在页面里
...
│   │   └── use-admin-accounts.ts            # 账号列表/详情/角色变更/成员 mutation
│   └── lib/
│       └── menu-registry.ts                 # 侧栏 admin-accounts 条目
...
通过 UI（`/admin/accounts/[id]` 平台角色下拉）或 SQL：
