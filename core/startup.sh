...
if command -v ocx >/dev/null 2>&1; then
  ...
  # ── Fix malformed opencode.jsonc generated by ocx init ──────────────────────
  # ocx init writes a template with a missing comma after "$schema", which is
  # invalid JSONC. Older OpenCode runtimes hard-fail on this during config
  # reload (e.g. after adding a custom LLM provider via the web UI), producing:
  #   "JSONC Input --- Errors --- ValueExpected at line 1, column 2"
