...
def find_repo_root() -> Path | None:
    ...
    for parent in current.parents:
        if (parent / "scripts" / "search_api_registry.py").exists() and (parent / "build").exists():
            return parent
    return None
...
def resolve_assets() -> tuple[Path, Path]:
    ...
    if root is None:
        raise SystemExit(
            "Could not locate local LingXing skill assets.\n"
            "Expected:\n"
            f"- {LOCAL_TARGET}\n"
            f"- {LOCAL_REGISTRY}\n"
    ...
    target = root / "scripts" / "search_api_registry.py"
