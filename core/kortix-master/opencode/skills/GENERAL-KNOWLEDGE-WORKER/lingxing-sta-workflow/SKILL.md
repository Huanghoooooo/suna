...
`shipmentPreView` 返回的是亚马逊/领星给出的分仓方案，不等于用户已经同意发货。调用 `confirmPlacementOption` 或 `setDeliveryService` 前必须先向用户展示发货卡片并取得明确确认。

发货卡片必须包含：
- 用户原始发货意图：店铺、MSKU、总数量、是否要求单个 FBA 仓库。
- 每个 `placementOptionId` 的方案摘要。
- 每个方案下所有 `shipmentInformationList` 的货件/目的仓信息、每仓数量、仓库数量、总数量。
- 明确提示：如果方案把货拆到多个 FBA 仓库，这不是“发到一个仓库”，必须让用户选择接受多仓、改数量/地址后重试，或停止。

禁止行为：
- 禁止只取 `placementOptionList[0]` 自动确认。
- 禁止只取 `shipmentInformationList[0]` 自动发第一个仓。
- 用户表达“发到一个 FBA 仓库/一个仓/一个目的仓”时，如果预览结果包含多个仓库或多个 shipment，必须暂停，不得调用 `confirmPlacementOption`。
- 用户只说“发 50 台”但没有明确接受分仓时，如果系统返回 5 个仓每仓 10 台，必须暂停并向用户确认，不能默认接受。
...
5. 创建 STA 任务并执行装箱
6. 查询 `shipmentPreView` 后生成发货卡片，展示目的仓数量、每仓数量、总数量和费用/方案信息
7. 用户明确确认发货卡片前，禁止确认货件方案或提交配送服务
8. 如果用户要求一个仓库但系统返回多仓方案，停止并询问用户是否接受多仓、改参数重试或取消
9. 每步执行后向用户报告进度
10. 遇到需要用户选择的地方（如入库配置选项、承运方式）暂停并询问
11. 最终输出：货件编号（FBA号）、货件名称、shipmentId
```
