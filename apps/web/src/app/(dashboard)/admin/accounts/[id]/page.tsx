...
function MembersTable({ members }: { members: AdminAccountMember[] }) {
  return (
    <section className="rounded-lg border">
      <div className="p-4 border-b">
        <div>
          <h2 className="text-base font-semibold flex items-center gap-2">
            <Crown className="w-4 h-4" /> 1:1 用户账号 User Account
          </h2>
          <p className="text-xs text-muted-foreground mt-1">
            每个用户只拥有自己的 personal account；成员关系在这里仅用于审计，不再支持把用户加入别人的账号。
            Each user owns exactly one personal account; membership is read-only here.
          </p>
        </div>
      </div>
      <Table>
          ...
          {members.length === 0 ? (
            ...
            members.map((m) => (
              <TableRow key={m.userId}>
                ...
                <TableCell>
                  <Badge variant="outline">{m.accountRole}</Badge>
                </TableCell>
              </TableRow>
            ))
          )}
